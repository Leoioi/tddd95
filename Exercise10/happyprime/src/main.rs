...
fn main() {
    ...
    for _ in 0..q {
        let i = next();
        let is_happy_prime_maybe = next();
        if happy_primes.contains(&is_happy_prime_maybe) {
            println!("{:?} {:?} YES", i, is_happy_prime_maybe);
        }
        else {
            println!("{:?} {:?} NO", i, is_happy_prime_maybe);
        }
    }
}

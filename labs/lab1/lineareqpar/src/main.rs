...
fn main() {
    ...
    loop { // loop forever!
        ...
        match gauss(&mut ab) {
            Ok(ans) => result.push_str( &(ans.iter().map(|opt| { 
                                match opt {
                                    Some(num) => num.to_string(), 
                                    None => String::from("?"), // If the ans is marked as none there could be multiple solutions
                                }
                            .to_string()
                            })
 ...
 */
fn gauss (ab: &mut Vec<Vec<f64>>) -> Result<Vec<Option<f64>>, GaussError> {
    let n = ab.len();
    ...
    let mut ans = vec![Some(0.0); n];


    // Extract the answer    
    for i in 0..n {
        if pivots[i] != -1 {
            ans[i] = Some(ab[pivots[i] as usize][n] / ab[pivots[i] as usize][i]);
        }
    }
    ...
    // This is the only real difference from the last lab
    for i in 0..n {
        if pivots[i] == -1 { // We have found some variable that is indeterminable, any other variables that contain this variable should also be marked as such 
            // i here is the variable that is indeterminable
            ans[i] = Option::None;
            for irow in 0..n {
                if ab[irow][i].abs() > EPS {
                    ans[irow] = Option::None ;
                }
            }
        }
    }

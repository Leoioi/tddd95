...
fn main() {
    ...
    loop {
        ...
        // Build adjacency list with minimal weights using HashMap
        let mut adj: Vec<HashMap<usize, usize>> = vec![HashMap::new(); n];
        for _ in 0..m {
            ...
            let w: usize = edge_line[2].parse().unwrap();
            adj[u]
                .entry(v)
                .and_modify(|old_w| *old_w = (*old_w).min(w))
                .or_insert(w);
        }

        // Convert to adjacency list of vectors for faster access
        let graph: Vec<Vec<(usize, usize)>> =
            adj.into_iter().map(|hm| hm.into_iter().collect()).collect();
...
*/
fn dijkstra(n: usize, start_node: usize, graph: Vec<Vec<(usize, usize)>>) -> Vec<(usize, Option<usize>)> {
    let mut best_cost: Vec<(usize, Option<usize>)> = vec![(usize::MAX, None); n];
    best_cost[start_node] = (0, Some(0));
    ...
    while let Some(Reverse((current_dist, u))) = queue.pop() {
        ...
        for &(neighbor_node, weigh) in &graph[u] {
            let new_weighs = current_dist + weigh;
